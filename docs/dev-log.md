...
첫 검표를 앞두고 스캐너 동선을 훑다 나온 것. 행사가 하나뿐이면 드러나지 않지만 둘 다 현장에서 복구가 안 되는 종류다.

**① 스캐너가 드랍을 구분하지 않았다.** `checkInTicket(token)`이 토큰만 받고 어느 스캐너에서 찍었는지 검증하지 않아, **A 공연 스캐너에서 B 공연 티켓을 찍으면 B가 입장 처리됐다.** 카운터는 드랍별로 집계되므로 숫자에도 안 잡혀 조용히 상태만 바뀐다. 같은 날 두 드랍을 돌리거나 지난 공연 티켓을 들고 온 관객이 있으면 터진다.

- `checkInTicket(token, dropId)` · `undoCheckIn(token, dropId)`로 스코프를 넘긴다.
- **판별 기준은 `order.dropId`.** 티어는 삭제 시 `SetNull`로 끊길 수 있어 드랍을 잃는다. `getCheckInStats`도 `ticketTier: { dropId }` → `order: { dropId }`로 통일했다 — 기준이 갈리면 티어가 삭제된 티켓이 분모에서만 빠져 "입장 12 / 11"이 나온다. 프로덕션 티켓 100건 전부 tier·order의 dropId가 일치하고 tier 누락 0건이라 현재 숫자는 변하지 않는다.
- 거절 메시지에 공연 제목을 넣었다("다른 공연의 입장권입니다 (…)"). 입장구에서 바로 안내할 수 있어야 한다.

**② 잘못 찍고 '다음'을 누르면 되돌릴 수 없었다.** 취소 버튼이 방금 체크인한 초록 패널에만 있었다. 다시 찍으면 주황 '이미 입장됨'이 뜨는데 거기엔 버튼이 없어, 현장에서 복구 경로가 사라진다. 주황 패널에도 `입장 취소`를 달았다.

**③ 그런데 취소 버튼 자체가 동작하지 않았다** (CodeRabbit이 잡았다). `handleUndo`가 디바운스 ref를 비우고 패널만 닫는데, 카메라는 계속 돌고 그 QR은 아직 눈앞에 있으므로 **다음 프레임에서 곧바로 재체크인**된다. 취소가 사실상 무효였다. 취소한 토큰을 억제 ref에 넣어 '다음'을 누르거나 다른 QR이 들어올 때까지 무시하고, `입장 취소됨` 패널을 띄워 무슨 일이 일어났는지 보이게 했다.

**검증**: type-check·biome·`next build` 통과. 실제 스캔은 첫 검표 때 확인.
...
**입장권 링크 복사도 함께** — 메일이 아예 닿지 않는 경우(스팸함·주소 오타)의 최종 수단. 어드민이 링크를 복사해 카톡·문자로 직접 전달한다. 이 과정에서 `ticket-token.ts`의 URL 빌더·QR 파서를 `ticket-url.ts`로 분리했다. 토큰 생성은 `node:crypto`에 의존하는 서버 전용 코드인데, 스캐너 뷰(클라이언트)가 이미 그 파일을 import하고 있었다.

**남은 한계**: Resend가 접수(202)한 뒤의 스팸함·바운스는 여전히 보이지 않는다. 주소 오타면 재발송해도 같은 곳으로 가므로, 그 케이스는 링크 복사로 우회하거나 주소 수정 기능이 필요하다.
