...
export function ArtworkSearchBar({ initialValue }: { initialValue: string }) {
  ...
  const submit = (e?: React.FormEvent) => {
    e?.preventDefault();
    const q = value.trim();
    router.push(q ? `/artworks?search=${encodeURIComponent(q)}` : '/artworks');
  };
  ...
  return (
    <form onSubmit={submit} aria-label="작품 검색">
